...
class Asteroid(ShowStats, gt.Entites, AffectedByGravity):
    def __init__(self, app: gt.Base, game, x: int = None, y: int = None, size: int = None):
        ...
        if zone == 0: #zone en haut
            x: int = x if x is not None else random.uniform(-self.size, game.map.sprite.get_size()[0])
            y: int = -self.size
        elif zone == 0: #zone en bas
            x: int = x if x is not None else random.uniform(-self.size, game.map.sprite.get_size()[0])
            y: int = game.map.sprite.get_size()[1]
        elif zone == 0: #zone a gauche
            x: int = -self.size
            y: int = y if y is not None else random.uniform(-self.size, game.map.sprite.get_size()[1])
        else: #zone a droite
            x: int = game.map.sprite.get_size()[0]
            y: int = y if y is not None else random.uniform(-self.size, game.map.sprite.get_size()[1])
